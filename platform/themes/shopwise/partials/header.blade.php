...
<body @if (BaseHelper::siteLanguageDirection()=='rtl' ) dir="rtl" @endif>
    ...
    $categories = get_product_categories(['status' => \Botble\Base\Enums\BaseStatusEnum::PUBLISHED], ['slugable',
    'children', 'children.slugable', 'icon'], [], true);
    } else {
    $categories = [];
    }
    @endphp
    ...
    <header
                    ...
                    <div class="col-md-6">
                        <div class="d-flex align-items-center justify-content-center justify-content-md-end">
                            ...
                            <div class="mr-3 choose-currency">
                                ...
                                @if(get_application_currency_id()==$currency->id)
                                <a href="{{ route('public.change-currency', $currency->title) }}"
                                    class="active"><span>{{ $currency->title }}</span></a>&nbsp;
                                @endif
                                @endforeach
                            </div>
                            @endif
                            <ul class="header_list">
                                <li><a href="{{ route('public.compare') }}"><i
                                            class="ti-control-shuffle"></i><span>{{ __('Compare') }}</span></a></li>
                                @if (!auth('customer')->check())
                                <li><a href="{{ route('customer.login') }}"><i
                                            class="ti-user"></i><span>{{ __('Login') }}</span></a></li>
                                @else
                                <li><a href="{{ route('customer.overview') }}"><i
                                            class="ti-user"></i><span>{{ auth('customer')->user()->name }}</span></a>
                                </li>
                                <li><a href="{{ route('customer.logout') }}"><i
                                            class="ti-lock"></i><span>{{ __('Logout') }}</span></a></li>
                                @endif
                            </ul>
                            @endif
                        </div>
            ...
            <div class="container">
                <div class="nav_block">
                    <a class="navbar-brand" href="{{ url('/') }}">
                        <img class="logo_dark" src="{{asset('img/logo/logo.png') }}"
                        ...
                        <form action="{{ route('public.products') }}" method="GET">
                            <div class="input-group">
                                    ...
                                    <div class="custom_select">
                                        <select name="categories[]" class="first_null">
                                            ...
                                            <option value="{{ $category->id }}" @if (in_array($category->id,
                                                request()->input('categories', []))) selected
                                ...
                                <input class="form-control" name="q" value="{{ request()->input('q') }}"
                                    placeholder="{{ __('Search Product') }}..." required type="text">
        ...
        <div
            class="bottom_header light_skin main_menu_uppercase bg_dark @if (url()->current() === url('')) mb-4 @endif">
            <div class="container">
                <div class="row">
                    <div class="col-lg-3 col-md-4 col-sm-6 col-4">
                        ...
                        <div class="categories_wrap">
                            ...
                            <div id="navCatContent"
                                class="@if (url()->current() === url('') && theme_option('collapsing_product_categories_on_homepage', 'no') == 'no') nav_cat @endif navbar collapse">
                                <ul>
                                    @foreach($categories as $category)
                                    @if ($loop->index < 10) <li @if ($category->children->count() > 0) class="dropdown
                                        dropdown-mega-menu" @endif>
                                        <a class="dropdown-item nav-link @if ($category->children->count() > 0) dropdown-toggler @endif"
                                            href="{{ $category->url }}" @if ($category->children->count() > 0)
                                        ...
                                        <li>
                                            <ul class="more_slide_open" style="display: none;">
                                                @endif
                                                <li @if ($category->children->count() > 0) class="dropdown
                                                    dropdown-mega-menu" @endif>
                                                    <a class="dropdown-item nav-link nav_item @if ($category->children->count() > 0) dropdown-toggler @endif"
                                                        href="{{ $category->url }}" @if ($category->children->count() >
                                                        0) data-toggle="dropdown" @endif>
                    ...
                    <div class="col-lg-9 col-md-8 col-sm-6 col-8">
                        <nav class="navbar navbar-expand-lg">
                            ...
                            <div class="collapse navbar-collapse mobile_side_menu" id="navbarSidetoggle">
                                {!! Menu::renderMenuLocation('main-menu', ['view' => 'menu', 'options' => ['class' =>
                                'navbar-nav']]) !!}
                            </div>
                            @if (is_plugin_active('ecommerce'))
                            <ul class="navbar-nav attr-nav align-items-center">
                                <li><a href="{{ route('public.wishlist') }}" class="nav-link btn-wishlist"><i
                                            class="linearicons-heart"></i><span
                                            class="wishlist_count">{{ !auth('customer')->check() ? Cart::instance('wishlist')->count() : auth('customer')->user()->wishlist()->count() }}</span></a>
                                </li>
                                ...
                                <li class="dropdown cart_dropdown"><a class="nav-link cart_trigger btn-shopping-cart"
                                        href="#" data-toggle="dropdown"><i class="linearicons-cart"></i><span
                                            class="cart_count">{{ Cart::instance('cart')->count() }}</span></a>
